...
```
#Research Question
## Using Power Transition Theory, which method of measuring state actor power best predicts the outcome of the proposed conflict between China and The United States?  
...
```{r}
# Pass the selection list from our Python enviroment. Note it automatically translate it as a vecorized list "c()" in our R enviroment.
WBD <-
  WBD %>%
...
```{r}
# Next I think it would be interesting to determine how well the six elements that comprise the CINC score and the score it self predict GDP via simple linear model.
...
#It shows that all six vectors are statistically signifigant therefore providing grounds to reject the Null based upon the determined signifigace.
...
```{r}
#The first grap[h we are going to make is the power trasisiton period between the United States and China. This has direct iomplications in Power transition theory. As the US is the hegemon of the international system, and China is an approching disatiosfied state (according to the theory of conditional anarchical international fourm). When these two states meet in power, the thoery posits that there will be a war between the two states. According to the current accepted prediction model, the CINC score the graphical illustration follows:
datesBeyond1960 %>%
